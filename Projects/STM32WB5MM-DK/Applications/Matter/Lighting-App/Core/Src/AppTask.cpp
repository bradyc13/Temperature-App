...
void UpdateTemperatureAttribute(){
	int16_t temperatureValue =TemperatureManager::sTemp.GetLevel();
	status = emberAfWriteAttribute(1, chip::ClusterId cluster,
            chip::AttributeId attributeID,&(uint8_t *)&temperatureValue,
			ZCL_INT16S_ATTRIBUTE_TYPE);
	if (status == Success) {
	    ChipLogProgress(Zcl, "Temperature attribute updated: %d", temperatureValue);
	} else {
	    ChipLogError(Zcl, "Failed to update temperature attribute: %d", status);
	}


}
void StartVirtualTimer(){
    // Configure TIM2 to trigger every 5 seconds
    __HAL_TIM_SET_AUTORELOAD(&htim2, 5000);  // 5000ms = 5 sec
...
CHIP_ERROR AppTask::Init() {
    ...
    // This line below might need to be edited
    ...
    err_2 = TemperatureMgr().Init();
    if (err_2!= CHIP_NO_ERROR){
    	APP_DBG("TemperatureMgr().Init() failed");
    	return err;
    }
    TemperatureMgr().SetCallbacks(ActionInitiated_Temp,ActionCompleted_Temp);
...
void AppTask::ActionInitiated_Temp(TemperatureManager::Action_t aAction) {
    // Placeholder for light action
	/*
#if (CFG_LCD_SUPPORTED == 1)
    UTIL_LCD_ClearStringLine(2);
...
#if (CFG_LCD_SUPPORTED == 1)
            char Message[11];
            snprintf(Message, sizeof(Message), "LED ON %d", TemperatureMgr().GetLevel());
            UTIL_LCD_DisplayStringAt(0, LINE(2), (uint8_t*) Message, CENTER_MODE);
#endif
            APP_DBG("Update level control %d", TemperaureMgr().GetLevel());
...
void AppTask::ActionCompleted_Temp(TemperatureManager::Action_t aAction) {
    // Placeholder for light action completed
	/*
    if (aAction == LightingManager::ON_ACTION) {
        APP_DBG("Light action on completed");
    ...
    */
	if (aAction==TemperatureManager::ON_ACTION){
		APP_DBG("Temperature action on completed");
	}
    if (sAppTask.mSyncClusterToButtonAction) {
        sAppTask.UpdateClusterState();
        sAppTask.mSyncClusterToButtonAction = false;
...
void AppTask::UpdateClusterState(void) {
    ...
    // Write new level value
    // The concept of the block of code below needs to be verified!
	status = Clusters::LevelControl::Attributes::CurrentLevel::Set(
	STM32_LIGHT_ENDPOINT_ID, TemperatureMgr().GetLevel());
	if (status != Protocols::InteractionModel::Status::Success){
		ChipLogError(NotSpecified, "ERR: updating level %x", status);
	}
}
