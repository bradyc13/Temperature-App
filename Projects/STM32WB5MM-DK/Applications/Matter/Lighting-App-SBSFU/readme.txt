...
Refer to Lighting-App for functionnal description.
...
To setup the application :
   ...
   g) To start OTA Test using Raspberrypi with Test Harness image, the following step is required:
	    ...
	    (sudo ./chip-tool otasoftwareupdaterequestor announce-otaprovider 1 0 1 0 3 0)
         - On Terminal_B : See results (BDX transfert). Save provider informations in NVM by pushing button B1 on Endevice at start of BDX transferts
...
In order to make the program work, you must do the following: 
 ...
 - Using CubeProgrammer: 
   - Flash BLE-Thread firmware (M0) : STM32WB-Matter-BLE-Thread\Projects\STM32WB_Copro_Wireless_Binaries\STM32WB5x\stm32wb5x_BLE_Thread_ForMatter_fw.bin at @ :  0x08096000 
   ...
   - Flash application software (M4) :  STM32WB-Matter-BLE-Thread\Projects\STM32WB5MM-DK\Applications\Matter\Lighting-App-SBSFU\Binary\SBSFU_Lighting-App-SBSFU.bin at @ : 0x08000000
 - To initiate OTA testing:
   - Modifiy the STM32WB-Matter-BLE-Thread\Projects\STM32WB5MM-DK\Applications\Matter\Lighting-App-SBSFU\Core\Inc\CHIPProjectConfig.h to change software version.
   - Generate MatterM4M0.bin file.
   - Install connectedhomeIP(CSA) environment.
   - Prepare the .ota file using Linux environment. 
   - The.ota file contains M0+M4 or M4 binar(ies) which is created with two tools. First one is CreateMatterBin.py used to merge M0 and M4 binaries in a .bin file with ST header. 
