                                       ...
                                       const uint8_t *pSource, uint32_t Length)
{
  ...
  if (BSP_QSPI_Write(0, (uint8_t *) pSource, (uint32_t) pDestination - EXTERNAL_FLASH_ADDRESS, Length) != BSP_ERROR_NONE)
  {
...
SFU_ErrorStatus SFU_LL_FLASH_EXT_Read(uint8_t *pDestination, const uint8_t *pSource, uint32_t Length)
{
  ...
  if (BSP_QSPI_Read(0, (uint8_t *) pDestination, (uint32_t) pSource - EXTERNAL_FLASH_ADDRESS, Length) != BSP_ERROR_NONE)
  {
    return SFU_ERROR;
  }
  else
...
SFU_ErrorStatus SFU_LL_FLASH_EXT_Compare(const uint8_t *pFlash, const uint32_t Pattern1, const uint32_t Pattern2, uint32_t Length)
{
  ...
  {
    if (BSP_QSPI_Read(0, (uint8_t *)&a_flash_buffer[0], flash + i - EXTERNAL_FLASH_ADDRESS, 16U) == BSP_ERROR_NONE)
    {
      for (j = 0U; j < 4U; j++)
      {
