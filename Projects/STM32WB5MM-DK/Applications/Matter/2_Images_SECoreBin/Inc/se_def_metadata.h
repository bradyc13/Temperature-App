 ...
 */
typedef uint32_t SE_FwStateTypeDef;

/*!< FW Image States */
#define FWIMG_STATE_INVALID   0UL /*!< Firmware image state: image is not valid (typically selftest has failed) and should not boot */
#define FWIMG_STATE_VALID     1UL /*!< Firmware image state: image is valid (selftest passed), image OK to boot if signature checks pass */
#define FWIMG_STATE_VALID_ALL 2UL /*!< Firmware image state: master image is valid (selftest passed), all other images are automatically validated */
#define FWIMG_STATE_SELFTEST  3UL /*!< Firmware image state: self tests to be executed. Image OK to boot one time if signature checks pass */
#define FWIMG_STATE_NEW       4UL /*!< Firmware image state: new image, has never run on the device */
...
typedef struct
{
  ...
  uint8_t  PartialFwTag[SE_TAG_LEN];/*!< Partial firmware Tag */
  uint8_t  Reserved[44U];           /*!< Reserved for future use: 44 extra bytes to have a header size of 192 bytes*/
  uint8_t  Certificates[2048U - 192U - 3U * 32U - 32U]; /*!< 2k - Header - FwImageState - PreHedareFingerprint */
  uint8_t  HeaderSignature[SE_HEADER_SIGN_LEN];  /*!< Signature of the full header message */
  uint8_t  FwImageState[3U][32U];  /*!< Firmware image state - see SE_FwStateTypeDef for details */
  uint8_t  PrevHeaderFingerprint[SE_FW_HEADER_FINGERPRINT_LEN]; /*!< Fingerprint of previous FW header (if this is an update, else 32*0x00) */
} SE_FwRawHeaderTypeDef;
