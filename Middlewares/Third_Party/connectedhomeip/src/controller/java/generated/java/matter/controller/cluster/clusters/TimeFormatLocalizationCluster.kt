...
class TimeFormatLocalizationCluster(
  ...
  class SupportedCalendarTypesAttribute(val value: List<UByte>?)

  sealed class SupportedCalendarTypesAttributeSubscriptionState {
    data class Success(val value: List<UByte>?) :
      SupportedCalendarTypesAttributeSubscriptionState()

    data class Error(val exception: Exception) : SupportedCalendarTypesAttributeSubscriptionState()
  ...
  suspend fun readHourFormatAttribute(): UByte {
    ...
    requireNotNull(attributeData) { "Hourformat attribute not found in response" }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { "Hourformat attribute not found in Node State update" }
  ...
  suspend fun readActiveCalendarTypeAttribute(): UByte? {
    ...
    requireNotNull(attributeData) { "Activecalendartype attribute not found in response" }
      ...
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) {
            "Activecalendartype attribute not found in Node State update"
          }
  ...
  suspend fun readSupportedCalendarTypesAttribute(): SupportedCalendarTypesAttribute {
    ...
    requireNotNull(attributeData) { "Supportedcalendartypes attribute not found in response" }
    ...
    return SupportedCalendarTypesAttribute(decodedValue)
  }

  suspend fun subscribeSupportedCalendarTypesAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<SupportedCalendarTypesAttributeSubscriptionState> {
    val ATTRIBUTE_ID: UInt = 2u
    val attributePaths =
      listOf(
        AttributePath(endpointId = endpointId, clusterId = CLUSTER_ID, attributeId = ATTRIBUTE_ID)
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(
            SupportedCalendarTypesAttributeSubscriptionState.Error(
              Exception(
                "Subscription terminated with error code: ${subscriptionState.terminationCause}"
              )
            )
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) {
            "Supportedcalendartypes attribute not found in Node State update"
          }
          ...
          decodedValue?.let { emit(SupportedCalendarTypesAttributeSubscriptionState.Success(it)) }
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(SupportedCalendarTypesAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }
  }
  ...
  companion object {
    private val logger = Logger.getLogger(TimeFormatLocalizationCluster::class.java.name)
