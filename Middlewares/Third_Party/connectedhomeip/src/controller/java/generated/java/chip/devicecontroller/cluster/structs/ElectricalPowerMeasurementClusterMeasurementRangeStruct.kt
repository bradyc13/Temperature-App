...
class ElectricalPowerMeasurementClusterMeasurementRangeStruct(
  ...
  val minTimestamp: Optional<ULong>,
  val maxTimestamp: Optional<ULong>,
    ...
    fun fromTlv(
      ...
      val minTimestamp =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_MIN_TIMESTAMP))) {
          Optional.of(tlvReader.getULong(ContextSpecificTag(TAG_MIN_TIMESTAMP)))
        } else {
          Optional.empty()
        }
      val maxTimestamp =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_MAX_TIMESTAMP))) {
          Optional.of(tlvReader.getULong(ContextSpecificTag(TAG_MAX_TIMESTAMP)))
        } else {
          Optional.empty()
        }
      val startSystime =
