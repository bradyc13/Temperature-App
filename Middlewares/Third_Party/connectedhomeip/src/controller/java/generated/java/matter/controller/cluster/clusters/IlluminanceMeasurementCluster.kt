...
class IlluminanceMeasurementCluster(
  ...
  sealed class MeasuredValueAttributeSubscriptionState {
    data class Success(val value: UShort?) : MeasuredValueAttributeSubscriptionState()
  ...
  sealed class MaxMeasuredValueAttributeSubscriptionState {
    data class Success(val value: UShort?) : MaxMeasuredValueAttributeSubscriptionState()
  ...
  sealed class LightSensorTypeAttributeSubscriptionState {
    data class Success(val value: UByte?) : LightSensorTypeAttributeSubscriptionState()

    data class Error(val exception: Exception) : LightSensorTypeAttributeSubscriptionState()

    object SubscriptionEstablished : LightSensorTypeAttributeSubscriptionState()
  }
  ...
  suspend fun subscribeLightSensorTypeAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<LightSensorTypeAttributeSubscriptionState> {
    val ATTRIBUTE_ID: UInt = 4u
    val attributePaths =
      listOf(
        AttributePath(endpointId = endpointId, clusterId = CLUSTER_ID, attributeId = ATTRIBUTE_ID)
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(
            LightSensorTypeAttributeSubscriptionState.Error(
              Exception(
                "Subscription terminated with error code: ${subscriptionState.terminationCause}"
              )
            )
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) {
            "Lightsensortype attribute not found in Node State update"
          }
          ...
          decodedValue?.let { emit(LightSensorTypeAttributeSubscriptionState.Success(it)) }
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(LightSensorTypeAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }
  }
