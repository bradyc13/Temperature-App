...
class OnOffCluster(private val controller: MatterController, private val endpointId: UShort) {
  ...
  sealed class StartUpOnOffAttributeSubscriptionState {
    data class Success(val value: UByte?) : StartUpOnOffAttributeSubscriptionState()

    data class Error(val exception: Exception) : StartUpOnOffAttributeSubscriptionState()

    object SubscriptionEstablished : StartUpOnOffAttributeSubscriptionState()
  }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { "Onoff attribute not found in Node State update" }
      ...
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) {
            "Globalscenecontrol attribute not found in Node State update"
          }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { "Ontime attribute not found in Node State update" }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { "Offwaittime attribute not found in Node State update" }
  ...
  suspend fun subscribeStartUpOnOffAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<StartUpOnOffAttributeSubscriptionState> {
    val ATTRIBUTE_ID: UInt = 16387u
    val attributePaths =
      listOf(
        AttributePath(endpointId = endpointId, clusterId = CLUSTER_ID, attributeId = ATTRIBUTE_ID)
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(
            StartUpOnOffAttributeSubscriptionState.Error(
              Exception(
                "Subscription terminated with error code: ${subscriptionState.terminationCause}"
              )
            )
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { "Startuponoff attribute not found in Node State update" }
          ...
          decodedValue?.let { emit(StartUpOnOffAttributeSubscriptionState.Success(it)) }
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(StartUpOnOffAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }
  }
