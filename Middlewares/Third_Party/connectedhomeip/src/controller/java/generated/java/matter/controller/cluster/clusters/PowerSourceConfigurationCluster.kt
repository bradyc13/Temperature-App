...
class PowerSourceConfigurationCluster(
  ...
  sealed class SourcesAttributeSubscriptionState {
    data class Success(val value: List<UShort>) : SourcesAttributeSubscriptionState()

    data class Error(val exception: Exception) : SourcesAttributeSubscriptionState()

    object SubscriptionEstablished : SourcesAttributeSubscriptionState()
  }
  ...
  suspend fun subscribeSourcesAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<SourcesAttributeSubscriptionState> {
    val ATTRIBUTE_ID: UInt = 0u
    val attributePaths =
      listOf(
        AttributePath(endpointId = endpointId, clusterId = CLUSTER_ID, attributeId = ATTRIBUTE_ID)
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(
            SourcesAttributeSubscriptionState.Error(
              Exception(
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { "Sources attribute not found in Node State update" }
          ...
          emit(SourcesAttributeSubscriptionState.Success(decodedValue))
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(SourcesAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }
  }
