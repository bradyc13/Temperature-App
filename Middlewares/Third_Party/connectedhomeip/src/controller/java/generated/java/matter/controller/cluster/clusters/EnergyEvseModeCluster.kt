...
class EnergyEvseModeCluster(
  private val controller: MatterController,
  private val endpointId: UShort
) {
  class ChangeToModeResponse(val status: UByte, val statusText: String?)

  class SupportedModesAttribute(val value: List<EnergyEvseModeClusterModeOptionStruct>)

  sealed class SupportedModesAttributeSubscriptionState {
    data class Success(val value: List<EnergyEvseModeClusterModeOptionStruct>) :
  ...
  suspend fun readSupportedModesAttribute(): SupportedModesAttribute {
    ...
    val decodedValue: List<EnergyEvseModeClusterModeOptionStruct> =
      buildList<EnergyEvseModeClusterModeOptionStruct> {
        tlvReader.enterArray(AnonymousTag)
        while (!tlvReader.isEndOfContainer()) {
          add(EnergyEvseModeClusterModeOptionStruct.fromTlv(AnonymousTag, tlvReader))
        }
        tlvReader.exitContainer()
      }
  ...
  companion object {
    private val logger = Logger.getLogger(EnergyEvseModeCluster::class.java.name)
    const val CLUSTER_ID: UInt = 157u
  }
}
