...
class BindingCluster(private val controller: MatterController, private val endpointId: UShort) {
  ...
  sealed class BindingAttributeSubscriptionState {
    data class Success(val value: List<BindingClusterTargetStruct>) :
      BindingAttributeSubscriptionState()

    data class Error(val exception: Exception) : BindingAttributeSubscriptionState()

    object SubscriptionEstablished : BindingAttributeSubscriptionState()
  }
  ...
  suspend fun readBindingAttribute(): BindingAttribute {
    ...
    requireNotNull(attributeData) { "Binding attribute not found in response" }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<BindingClusterTargetStruct> =
      buildList<BindingClusterTargetStruct> {
        tlvReader.enterArray(AnonymousTag)
        while (!tlvReader.isEndOfContainer()) {
          add(BindingClusterTargetStruct.fromTlv(AnonymousTag, tlvReader))
        }
        tlvReader.exitContainer()
      }

    return BindingAttribute(decodedValue)
  }
  ...
  suspend fun subscribeBindingAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<BindingAttributeSubscriptionState> {
    val ATTRIBUTE_ID: UInt = 0u
    val attributePaths =
      listOf(
        AttributePath(endpointId = endpointId, clusterId = CLUSTER_ID, attributeId = ATTRIBUTE_ID)
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(
            BindingAttributeSubscriptionState.Error(
              Exception(
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { "Binding attribute not found in Node State update" }
          ...
          emit(BindingAttributeSubscriptionState.Success(decodedValue))
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(BindingAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }
  }
  ...
  companion object {
    private val logger = Logger.getLogger(BindingCluster::class.java.name)
