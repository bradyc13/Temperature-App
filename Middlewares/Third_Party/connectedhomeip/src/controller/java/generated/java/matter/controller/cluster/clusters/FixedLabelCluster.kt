...
class FixedLabelCluster(private val controller: MatterController, private val endpointId: UShort) {
  ...
  sealed class LabelListAttributeSubscriptionState {
    data class Success(val value: List<FixedLabelClusterLabelStruct>) :
      LabelListAttributeSubscriptionState()

    data class Error(val exception: Exception) : LabelListAttributeSubscriptionState()

    object SubscriptionEstablished : LabelListAttributeSubscriptionState()
  }
  ...
  suspend fun readLabelListAttribute(): LabelListAttribute {
    ...
    requireNotNull(attributeData) { "Labellist attribute not found in response" }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<FixedLabelClusterLabelStruct> =
      buildList<FixedLabelClusterLabelStruct> {
        tlvReader.enterArray(AnonymousTag)
        while (!tlvReader.isEndOfContainer()) {
          add(FixedLabelClusterLabelStruct.fromTlv(AnonymousTag, tlvReader))
        }
        tlvReader.exitContainer()
      }

    return LabelListAttribute(decodedValue)
  }

  suspend fun subscribeLabelListAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<LabelListAttributeSubscriptionState> {
    val ATTRIBUTE_ID: UInt = 0u
    val attributePaths =
      listOf(
        AttributePath(endpointId = endpointId, clusterId = CLUSTER_ID, attributeId = ATTRIBUTE_ID)
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(
            LabelListAttributeSubscriptionState.Error(
              Exception(
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { "Labellist attribute not found in Node State update" }
          ...
          emit(LabelListAttributeSubscriptionState.Success(decodedValue))
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(LabelListAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }
  }
  ...
  companion object {
    private val logger = Logger.getLogger(FixedLabelCluster::class.java.name)
