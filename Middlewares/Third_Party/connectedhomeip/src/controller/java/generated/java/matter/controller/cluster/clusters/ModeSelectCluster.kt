...
class ModeSelectCluster(private val controller: MatterController, private val endpointId: UShort) {
  class StandardNamespaceAttribute(val value: UShort?)

  sealed class StandardNamespaceAttributeSubscriptionState {
    data class Success(val value: UShort?) : StandardNamespaceAttributeSubscriptionState()

    data class Error(val exception: Exception) : StandardNamespaceAttributeSubscriptionState()

    object SubscriptionEstablished : StandardNamespaceAttributeSubscriptionState()
  }
  ...
  sealed class SupportedModesAttributeSubscriptionState {
    data class Success(val value: List<ModeSelectClusterModeOptionStruct>) :
  ...
  suspend fun readStandardNamespaceAttribute(): StandardNamespaceAttribute {
    ...
    requireNotNull(attributeData) { "Standardnamespace attribute not found in response" }
    ...
    return StandardNamespaceAttribute(decodedValue)
  }

  suspend fun subscribeStandardNamespaceAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<StandardNamespaceAttributeSubscriptionState> {
    val ATTRIBUTE_ID: UInt = 1u
    val attributePaths =
      listOf(
        AttributePath(endpointId = endpointId, clusterId = CLUSTER_ID, attributeId = ATTRIBUTE_ID)
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(
            StandardNamespaceAttributeSubscriptionState.Error(
              Exception(
                "Subscription terminated with error code: ${subscriptionState.terminationCause}"
              )
            )
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) {
            "Standardnamespace attribute not found in Node State update"
          }
          ...
          decodedValue?.let { emit(StandardNamespaceAttributeSubscriptionState.Success(it)) }
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(StandardNamespaceAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }
  }
  ...
  suspend fun readSupportedModesAttribute(): SupportedModesAttribute {
    ...
    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<ModeSelectClusterModeOptionStruct> =
      buildList<ModeSelectClusterModeOptionStruct> {
        tlvReader.enterArray(AnonymousTag)
        while (!tlvReader.isEndOfContainer()) {
          add(ModeSelectClusterModeOptionStruct.fromTlv(AnonymousTag, tlvReader))
        }
  ...
  companion object {
    private val logger = Logger.getLogger(ModeSelectCluster::class.java.name)
